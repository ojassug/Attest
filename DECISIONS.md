...
## 2026-09-08 · The AgentCore runtime stops at Gate 1, and P7 was pulled forward

**Decision (P7-S3).** `agent_runtime.py` exposes the whole pipeline behind one `@app.entrypoint`
handler on `BedrockAgentCoreApp`: note in, assembled packet out.

**It never emits an artifact.** The response carries the packet plus the `content_hash` a clinician
must approve, and stops there. A headless deployment is precisely where human approval would
otherwise decay into a UI convention — there is no interface to remember to ask — so the runtime
returning `approval_required: true` is the product rule surviving contact with automation.
`test_runtime_does_not_emit_an_artifact` and `test_response_carries_the_hash_to_be_approved` pin it.

**Never raises to the caller.** A deployed service that answers a bad payload with a 500 tells the
caller nothing actionable, so failures return `{"error": ...}` with the reason — including the case
that matters most: no policy pack covering the CPT returns an error naming the escalation, never a
packet, because absence of a policy is not evidence that no authorization is needed.

**Verified with no AWS and no API key.** The pipeline replays from cassettes, so
`python agent_runtime.py` plus the DoD's `curl` returns a complete packet in ~0.23s on a machine
with no credentials at all. That is what makes the runtime demonstrable to a judge who has cloned
the repo and nothing else.

### `pythonpath = ["."]` — the gate could not import the runtime

`scripts/verify.sh` invokes the `pytest` console script, which does **not** put the working
directory on `sys.path`; `python -m pytest` does. AgentCore requires `agent_runtime.py` at the repo
root, so the tests passed when run directly and failed collection under the gate.

Fixed by declaring `pythonpath = ["."]` in `[tool.pytest.ini_options]` rather than by moving the
file or patching `sys.path` in a conftest. This is the same failure shape as the `verify.sh`
bin/Scripts bug: works under one invocation, silently fails under the other, and the one that
fails is the one the protocol trusts.

### Out of order, deliberately

PLAN.md says not to start P7 until `./scripts/verify.sh P6` is green. P7-S3 was taken at the user's
direction because AgentCore deployment is the priority. **P5 and P6 remain unbuilt and are required
for a submittable product; P7 is upside.** Recorded so the ordering is a visible decision rather
than something a later session has to reverse-engineer from the board.

**P7-S4 is blocked on machine setup, not on code:** the AWS CLI, Docker, and the `agentcore` CLI
(`bedrock-agentcore-starter-toolkit`, which is not a declared dependency) are all absent, and
`agentcore configure` / `launch` need all three plus credentials.

...
def mark_note_evidence(note_text: str, coverage: CriteriaCoverage) -> str:
    """Render note_text with verified evidence spans highlighted and attributed to criteria.

    Only verified spans are marked. Overlapping spans are segmented cleanly so that no
    underlying characters are lost, duplicated, or corrupted.
    """
    items: list[tuple[int, int, str]] = []
    for verdict in coverage.verdicts:
        for span in verdict.spans:
            if span.verified and 0 <= span.start < span.end <= len(note_text):
                items.append((span.start, span.end, verdict.criterion_id))

    if not items:
        return note_text

    points = {0, len(note_text)}
    ...
    for i in range(len(sorted_points) - 1):
        ...
        chunk_text = note_text[p_start:p_end]
        if not chunk_text:
            continue

        matching_crit = [crit for (s, e, crit) in items if s <= p_start and p_end <= e]
        crit_list = []
        seen = set()
        for c in matching_crit:
            if c not in seen:
                seen.add(c)
                crit_list.append(c)

        if crit_list:
            crit_label = ", ".join(crit_list)
            chunks.append(
                f'<mark style="background-color: #dbeafe; color: #1e3a8a; padding: 2px 4px; border-radius: 3px;" '
                f'title="Criterion: {crit_label}"><b>[{crit_label}]</b> {chunk_text}</mark>'
            )
        else:
...
if uploaded is None:
    ...
    with st.expander("The clinical note", expanded=False):
        if "coverage" in state:
            st.caption("Verified evidence quotes are marked in place, attributed to their criteria.")
            marked = mark_note_evidence(note_text, state["coverage"])
            st.markdown(f'<div class="clinical-note-content">\n\n{marked}\n\n</div>', unsafe_allow_html=True)
        else:
